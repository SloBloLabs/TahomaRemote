...
void LEDDriver::process() {
    ...
    if(USE_DMA) {
        ...
        for(_curChip = 0; _curChip < NUM_PWM_LED_CHIPS; ++_curChip) {
            ...
            if(status == ERROR) {
                DEBUG_LOG("\n[LEDDriver.process] error writing to register");
            }
        }
    }
}
...
void LEDDriver::notifyTxError() {
    DEBUG_LOG("\n[LEDDriver.notifyTxError] LED Tx Error!");
...
void LEDDriver::resetChip(uint32_t chipNumber) {
    ...
    if(status == ERROR) {
        DEBUG_LOG("\n[LEDDriver.resetChip] error writing to register");
    }
#ifdef osWaitForever
    osDelay(1);
#else
...
ErrorStatus LEDDriver::writeRegistersDMA(uint8_t chipNumber, uint8_t startRegister, uint8_t* registerValues, uint32_t count) {
    ...
    if(status == SUCCESS) {
        ...
        if(ret == ERROR || !LL_I2C_IsActiveFlag_TXE(LED_I2C)) {
            DEBUG_LOG("\n[LEDDriver.writeRegistersDMA] TX buffer not empty, aborting");
            return ERROR;
        }
...
void LEDDriver::checkStatus() {
    if(LL_I2C_IsActiveFlag_SB(LED_I2C))    { DEBUG_LOG("\n[LEDDriver.checkStatus] SB active"); }
    if(LL_I2C_IsActiveFlag_ADDR(LED_I2C))  { DEBUG_LOG("\n[LEDDriver.checkStatus] ADDR active"); }
    if(LL_I2C_IsActiveFlag_BTF(LED_I2C))   { DEBUG_LOG("\n[LEDDriver.checkStatus] BTF active"); }
    if(LL_I2C_IsActiveFlag_ADD10(LED_I2C)) { DEBUG_LOG("\n[LEDDriver.checkStatus] ADD10 active"); }
    if(LL_I2C_IsActiveFlag_STOP(LED_I2C))  { DEBUG_LOG("\n[LEDDriver.checkStatus] STOPF active"); }
    if(LL_I2C_IsActiveFlag_RXNE(LED_I2C))  { DEBUG_LOG("\n[LEDDriver.checkStatus] RxNE active"); }
    if(LL_I2C_IsActiveFlag_TXE(LED_I2C))   { DEBUG_LOG("\n[LEDDriver.checkStatus] TxE active"); }
    if(LL_I2C_IsActiveFlag_BERR(LED_I2C))  { DEBUG_LOG("\n[LEDDriver.checkStatus] BERR active"); }
    if(LL_I2C_IsActiveFlag_ARLO(LED_I2C))  { DEBUG_LOG("\n[LEDDriver.checkStatus] ARLO active"); }
    if(LL_I2C_IsActiveFlag_AF(LED_I2C))    { DEBUG_LOG("\n[LEDDriver.checkStatus] AF active"); }
    if(LL_I2C_IsActiveFlag_OVR(LED_I2C))   { DEBUG_LOG("\n[LEDDriver.checkStatus] OVR active"); }
}
...
ErrorStatus LEDDriver::sendData(uint8_t data) {
    ...
    if(ret == ERROR || !LL_I2C_IsActiveFlag_TXE(LED_I2C)) {
        DEBUG_LOG("\n[LEDDriver.sendData] TX buffer not empty, aborting");
        return ERROR;
    }
